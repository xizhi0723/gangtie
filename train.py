...
config = {
    ...
    "batch_size": 4,
    "num_epochs": 100,
    "learning_rate": 1e-4,         # <--- MODIFICATION: Lower learning rate for fine-tuning
    "img_size": (256, 256),
    "num_classes": 4,
...
class DiceLoss(nn.Module):
    ...
    def _one_hot_encoder(self, input_tensor):
        ...
        for i in range(self.n_classes):
            temp_prob = input_tensor == i; tensor_list.append(temp_prob.unsqueeze(1))
        return torch.cat(tensor_list, dim=1)
    def _dice_loss(self, score, target):
        target = target.float(); smooth = 1e-5
        intersect = torch.sum(score * target)
        y_sum = torch.sum(target * target); z_sum = torch.sum(score * score)
...
def train_model(train_loader, val_loader):
    ...
    # <--- MODIFICATION: Use AdamW optimizer
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for images, masks in train_loader:
            ...
            # <--- MODIFICATION: Give more weight to Dice loss
...
def predict_and_save():
    ...
    with torch.no_grad():
        for img_name in test_images:
            ...
            # <--- MODIFICATION: Test-Time Augmentation (TTA) ---
            # 1. Original image prediction
            image_tensor_orig = transform(image=image)['image'].unsqueeze(0).to(config["device"])
            output_orig = model(image_tensor_orig)

            # 2. Horizontally flipped image prediction
            image_flipped = np.fliplr(image).copy()
            image_tensor_flipped = transform(image=image_flipped)['image'].unsqueeze(0).to(config["device"])
            output_flipped = model(image_tensor_flipped)
            
            # 3. Flip the prediction back
            output_flipped = torch.flip(output_flipped, dims=[3])

            # 4. Average the predictions
            # We average the probabilities (after softmax), not the raw outputs (logits)
            final_output = (torch.softmax(output_orig, dim=1) + torch.softmax(output_flipped, dim=1)) / 2
    ...
    print("Prediction with TTA results saved to result.zip")
...
if __name__ == "__main__":
    ...
    train_transform = alb.Compose([
        alb.Resize(*config["img_size"]),
        alb.HorizontalFlip(p=0.5), alb.VerticalFlip(p=0.5), alb.RandomRotate90(p=0.5),
        alb.RandomBrightnessContrast(p=0.2), alb.GaussNoise(p=0.2),
        alb.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
        ToTensorV2()
    ])
